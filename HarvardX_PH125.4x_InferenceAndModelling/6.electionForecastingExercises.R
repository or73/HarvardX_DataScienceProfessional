# Election Forecasting Exercises
...
#
# INSTRUCTIONS
# Use pipes %>% to pass the poll object on to the mutate function, which creates
#    new variables.
# Create a variable called X_hat that contains the estimate of the proportion of
#    Clinton voters for each poll.
# Create a variable called se that contains the standard error of the spread.
# Calculate the confidence intervals using the qnorm function and your
#    calculated se.
# Use the select function to keep the following columns: state, startdate,
#    enddate, pollster, grade, spread, lower, upper.
...
#    contain the actual spread using the following steps.
# Use the mutate function to create a new variable called hit that contains a
#    logical vector for whether the actual_spread falls between the lower and
#    pper confidence intervals.
...
p_hits <- cis %>% mutate(hit = ( lower <= actual_spread & upper >= actual_spread)) %>% summarize(mean(hit))
p_hits
...
# Now find the proportion of hits for each pollster. Show only pollsters with
#    more than 5 polls and order them from best to worst. Show the number of
#    polls conducted by each pollster and the FiveThirtyEight grade of each pollster.
#
# INSTRUCTIONS
# Create an object called p_hits that contains the proportion of intervals that
#    contain the actual spread using the following steps.
...
# Summarize the proportion of values in hit that are true as a variable called
#    proportion_hits. Also create new variables for the number of polls by each
#    pollster using the n() function and the grade of each poll.
...
p_hits <- cis %>% mutate(hit = lower <= actual_spread.x & upper >= actual_spread.x) %>%
    group_by(pollster) %>%
    filter(n() >=  5) %>%
...
# Summarize the proportion of values in hit that are true as a variable called
#    proportion_hits. Also create new variables for the number of polls in each
...
# Using ggplot, set the aesthetic with state as the x-variable and proportion of
#     hits as the y-variable.
# Use geom_bar to indicate that we want to plot a barplot. Specifcy stat = "identity"
#    to indicate that the height of the bar should match the value.
# Use coord_flip to flip the axes so the states are displayed from top to bottom
#    and proportions are displayed from left to right.
...
# Even if a forecaster's confidence interval is incorrect, the overall predictions
#    will do better if they correctly called the right winner.
#
# Add two columns to the cis table by computing, for each poll, the difference
#    between the predicted spread and the actual spread, and define a column hit
...
# Create an object called p_hits that contains the proportion of instances when
#    the sign of the actual spread matches the predicted spread for states with
#    more than 5 polls.
#
# Make a barplot based on the result from the previous exercise that shows the
#    proportion of times the sign of the spread matched the actual result for
#    the data in p_hits.
#
# INSTRUCTIONS
# Use the group_by function to group the data by state.
...
#    25% of the time. Wisconsin got every single poll wrong. In Pennsylvania and
...
# We see that, at the state level, the median error was slightly in favor of
#    Clinton. The distribution is not centered at 0, but at 0.037. This value
#    represents the general bias we described in an earlier section.
#
# Create a boxplot to examine if the bias was general to all states or if it
#    affected some states differently. Filter the data to include only pollsters
#    with grades B+ or higher.
#
# INSTRUCTIONS
# Use the filter function to filter the data for polls with grades equal to A+,
#    A, A-, or B+.
...
# Some of these states only have a few polls. Repeat the previous exercise to
#    plot the errors for each state, but only include states with five good polls
#    or more.
#
# INSTRUCTIONS
# Use the filter function to filter the data for polls with grades equal to A+,
