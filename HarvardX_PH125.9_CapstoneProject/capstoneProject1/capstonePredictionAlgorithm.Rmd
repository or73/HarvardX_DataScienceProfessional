...
\small
```{r edx.test.users.movies.ratings, highlight=TRUE, echo=FALSE, out.width='65%'}
summary(edx.test)
```   
\pagebreak   
\normalsize   

# Methods & Analysis - Visualize the Importance of Variables   
## All Data   
...
rm(movies.10.best.rated, movies.10.worst.rated.movies)
```       
...
movies.lambda %>%
    ...
    geom_point(shape = 1,
               alpha = 0.5)
rm(lambda, movies.lambda)
```
   
After regularization procedure, we should have a different information, let's see it:   
   
10 best rated movies after regularization:   
```{r regularization.10.best.movies, echo=FALSE}
...
movies.top.10.best.regularized <- edx.test %>%
    ...
    dplyr::select(title, 
                  ...
                  n) %>%
    slice(1:10)
table(movies.top.10.best.regularized, 
      'Top 10 of Best Regularized Movies',
      'small')
```      

10 worst rated movies after regularization:   
```{r regularization.10.worst.movies, echo=FALSE}
...
movies.top.10.worst.regularized <- edx.test %>%
    ...
    dplyr::select(title, 
                  ...
                  n) %>%
    slice(1:10)
table(movies.top.10.worst.regularized, 
      'Top 10 of Worst Regularized Movies',
      'small')
rm(movies.top.10.best.regularized, movies.top.10.worst.regularized)
```   
   
Is time to validate if regularization represents some better performance.  And, the RMSEs table comparisson:    
```{r regularization.predict.movies.bias, echo=FALSE}
# ---------------------------------------------- Prediction for Movies Bias after regularization
predict.rating <- edx.test %>%
    left_join(movies.reg.avg, 
              by = 'movieId') %>%
    mutate(pred = mu.hat + b.i) %>%
    .$pred

# ---------------------------------------------- RMSE movies Bias after Regularization
RMSE.movies.regularization.bias <- RMSE(predict.rating, 
                                        edx.test$rating)
table.RMSE.comparisson <- bind_rows(table.RMSE.comparisson,
...
We can see a better perfomance betwee \textcolor{red}{Movies Bias} and \textcolor{red}{Regularization Movies Bias}.  And, we can validate that in the \textcolor{red}{Regularization Movies Bias} top 10 best and top 10 worst movies have a more logical ranking, according to the historical information.   

## Identify Lambda   
Now we must identify a `lambda` value to find the lowest RMSE, to identify it, we will use a function with a sequence of number from 0 to 7, applied to movies bias.  And, we will se in a graph the $\lambda$ which produces the lowest RMSE.   
   
```{r cross.validation.lambda, echo=FALSE}
# ---------------------------------------------- Cross validation to choose a lambda
# Identify the best lambda *from 0 to 7 at sequence of 0.25), over the movies bias, and the lowest value will be choosed
...
We will find the $\lambda$ which provides the smallest RMSE:   
     
```{r regularization.users.movies.small.rmse, echo=FALSE}
# ---------------------------------------------- Regularization users & movies Bias
...
The best $\lambda$ is: `r best.lambda`.   

```{r best.rmse, echo=FALSE}
# ---------------------------------------------- Best RMSE
table.RMSE.comparisson <- bind_rows(table.RMSE.comparisson,
...
# Conclusion   
Machine Learning has been used to create a `Movie Recommendation System`, and deduce that a join between `Users & Movies Bias`, produce the best RMSE improvement for our System.  The best and lowest obtained RMSE = `r min(RMSE.function.reg)`}
