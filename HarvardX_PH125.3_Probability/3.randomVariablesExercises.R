...
#    and black pocket is associated with a number from 1 to 36. The two
#    remaining green slots feature "0" and "00". Players place bets on which
#    pocket they think a ball will land in after the wheel is spun. Players can
#    bet on a specific number (0, 00, 1-36) or color (red, black, or green).
#
# What are the chances that the ball lands in a green pocket?
#
# INSTRUCTIONS
# Define a variable p_green as the probability of the ball landing in a green
#    pocket.
...
# Use the prob = argument in the sample function to specify a vector of
#    probabilities for returning each of the values contained in the vector of
...
# The variables 'green', 'black', and 'red' contain the number of pockets for
#    each color
green <- 2
black <- 18
red <- 18
...
# In American roulette, the payout for winning on green is $17. This means that
#    if you bet $1 and it lands on green, you get $17 as a prize.In the previous
#    exercise, you created a model to predict your winnings from betting on green.
#
...
#    (p_not_green), calculate the expected outcome of a bet that the ball will
#    land in a green pocket.
...
# The standard error of a random variable X tells us the difference between a
#    random variable and its expected value. You calculated a random variable X
...
sqrt( 1 ) * 18 * sqrt( p_green * p_not_green )
...
#
# INSTRUCTIONS
# Using the chances of winning $17 (p_green) and
#       the chances of losing $1 (p_not_green),
# calculate the expected outcome of a bet that the ball will land in a green
#    pocket over 1,000 bets.
...
# You generated the expected value of S, the outcomes of 1,000 bets that the
#    ball lands in the green pocket, in the previous exercise.
#
...
# Compute the standard error of the sum of 1,000 outcomesN
